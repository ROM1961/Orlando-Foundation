...
async def get_relayer_info(user_id: str = Depends(get_current_user)):
...
async def get_relayer_status(user_id: str = Depends(get_current_user)):
...
@router.get("/chains")
async def get_supported_chains(user_id: str = Depends(get_current_user)):
